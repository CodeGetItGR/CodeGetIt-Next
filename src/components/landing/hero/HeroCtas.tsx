  ...
  return (
    ...
    >
      <div className="flex flex-row items-center gap-3">
        {/* Primary — magnetic + button-in-button arrow + cta-polish sheen */}
        <MagneticCta
          href="#contact"
          className={cn(
            'group cta-polish',
            'inline-flex cursor-pointer items-center gap-2 rounded-full border-none',
            'bg-brand-600 py-2.5 pr-2 pl-6 no-underline',
      ...
      {copy.note && (
        <p className="text-[12px] font-medium tracking-wide text-slate-400">{copy.note}</p>
      )}
    </motion.div>
  );
}
